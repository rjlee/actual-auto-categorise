...
function scheduleClassification(verbose) {
  ...
  if (disableCron) {
    logger.info({ job: 'classification' }, 'Cron scheduling disabled via DISABLE_CRON_SCHEDULING');
    return;
  }
  ...
  cron.schedule(schedule, async () => {
    ...
    if (running) {
      logger.warn({ ts }, 'Skipping scheduled classification run: previous run still in progress');
      return;
    }
    running = true;
...
function scheduleTraining(verbose) {
  const disableCron = process.env.DISABLE_CRON_SCHEDULING === 'true'
    || config.DISABLE_CRON_SCHEDULING === true;
  if (disableCron) {
    logger.info({ job: 'training' }, 'Cron scheduling disabled via DISABLE_CRON_SCHEDULING');
    return;
  }
  ...
  cron.schedule(schedule, async () => {
    const ts = new Date().toISOString();
    if (running) {
      logger.warn({ ts }, 'Skipping scheduled training: previous run still in progress');
      return;
    }
    running = true;
...
async function runDaemon({ verbose, ui, httpPort }) {
  const explicitPort = typeof config.httpPort !== 'undefined'
